...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubReviews = onSnapshot(collection(db, 'reviews'), snap => setReviews(snap.docs));
    return () => { unsubPosts(); unsubPolls(); unsubReviews(); };
  }, []);
  ...
  return (
    <main style={PAGE}>
      ...
      <div style={SECTION}>
        ...
        <div style={TABLE}>
          <div style={TABLE_HEAD}>
            <span>Title</span><span>Tags</span><span>Status</span><span>Action</span>
          </div>
          {posts.slice(0, 4).map(doc => {
            const p = doc.data();
            return (
              <div key={doc.id} style={TABLE_ROW}>
                <span style={{ fontWeight: 600, fontSize: '13px' }}>{p.title || '—'}</span>
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{(p.tags || []).join(', ') || '—'}</span>
                <span><span style={p.published ? BADGE_LIVE : BADGE_DRAFT}>{p.published ? 'Published' : 'Draft'}</span></span>
                <span><Link href={`/admin/posts/${doc.id}`} style={ACTION_LINK}>Edit</Link></span>
              </div>
            );
          })}
          {posts.length === 0 && <div style={EMPTY}>No posts yet.</div>}
        </div>
      </div>
      ...
      <div style={SECTION}>
        <div style={SEC_HEADER}>
          <div style={SEC_TITLE}>Recent Reviews</div>
          <Link href="/admin/reviews" style={SEC_LINK}>View all →</Link>
        </div>
        <div style={TABLE}>
          <div style={TABLE_HEAD}>
            <span>Train</span><span>Route</span><span>Status</span><span>Action</span>
          </div>
          {reviews.slice(0, 4).map(doc => {
            const r = doc.data();
            const s = r.scores || {};
            ...
            return (
              <div key={doc.id} style={TABLE_ROW}>
                ...
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>
                  {r.route || '—'} {avg !== '—' && <span style={{ color: '#1E90FF', marginLeft: '6px' }}>★ {avg}</span>}
                </span>
                <span><span style={r.published ? BADGE_LIVE : BADGE_DRAFT}>{r.published ? 'Published' : 'Draft'}</span></span>
                <span><Link href={`/admin/reviews/${doc.id}`} style={ACTION_LINK}>Edit</Link></span>
              </div>
            );
          })}
      ...
      <div style={SECTION}>
        ...
        <div style={TABLE}>
          <div style={TABLE_HEAD}>
            <span>Question</span><span>Options</span><span>Status</span><span>Action</span>
          </div>
          {polls.slice(0, 4).map(doc => {
            const p = doc.data();
            const optionsRaw = p.options || [];
            const opts = Array.isArray(optionsRaw) ? optionsRaw : Object.values(optionsRaw);
            return (
              <div key={doc.id} style={TABLE_ROW}>
                <span style={{ fontWeight: 600, fontSize: '13px' }}>{p.question || '—'}</span>
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{opts.length} options</span>
                <span><span style={p.isActive ? BADGE_LIVE : BADGE_DRAFT}>{p.isActive ? 'Active' : 'Expired'}</span></span>
                <span><Link href={`/admin/polls/${doc.id}`} style={ACTION_LINK}>Edit</Link></span>
              </div>
            );
          })}
          {polls.length === 0 && <div style={EMPTY}>No polls yet.</div>}
        </div>
      </div>
    </main>
  );
...
const TABLE_HEAD = { display: 'grid', gridTemplateColumns: '2fr 1.5fr 100px 80px', gap: '1rem', padding: '12px 20px', fontSize: '9px', fontWeight: 900, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', borderBottom: '1px solid rgba(255,255,255,0.06)' };
const TABLE_ROW  = { display: 'grid', gridTemplateColumns: '2fr 1.5fr 100px 80px', gap: '1rem', padding: '14px 20px', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)' };
