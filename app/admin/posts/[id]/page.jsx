    ...
    <main style={PAGE}>
      <div style={HEADER}>
        <div>
          ...
          <h1 style={HEADING}>{loading ? 'Loading…' : 'Edit Post'}</h1>
          {post?.title && <div style={SUBTITLE}>{post.title}</div>}
        </div>
        <Link href="/admin/posts" style={BACK}>← Back to Posts</Link>
      </div>
      {loading ? (
        <div style={EMPTY}>Loading post…</div>
      ) : !post ? (
        <div style={EMPTY}>Post not found.</div>
      ) : (
        <PostForm existing={post} id={id} />
      )}
    </main>
  );
}
...
const SUBTITLE = { fontSize: '13px', color: 'rgba(255,255,255,0.35)', marginTop: '4px' };
